...
use crate::qos::ledger;
use crate::ui::{ClaimRow, RunRow, StatusView, Theme, render_status};
...
async fn snapshot(client: &Client) -> Result<StatusView, String> {
    ...
    Ok(StatusView {
        context: crate::cluster_config::active_context().unwrap_or_else(|| "?".into()),
        server,
        nodes: crate::pipeline::cluster::node_summary(&nodes.items),
        allocatable: crate::pipeline::cluster::cluster_allocatable(&nodes.items),
        capacity: crate::pipeline::cluster::total_allocatable(&nodes.items),
...
fn project_starts(claims: Vec<Beacon>, runs: &[RunRow], nodes: &[Node]) -> Vec<ClaimRow> {
    let allocatable = crate::pipeline::cluster::cluster_allocatable(nodes);
    ...
    runs.iter()
        ...
        .map(|r| {
            format!(
                ...
                r.beacon.reserve.mem_bytes / crate::qos::GIB,
