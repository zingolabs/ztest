...
use crate::resource::{self, Graph, NodeId, NodeState, Progress};
...
/// [`rate::Window`](crate::rate::Window) that smooths them
...
struct TransferRegistry {
    ...
    rates: std::collections::BTreeMap<NodeId, crate::rate::Window<u64>>,
}
...
impl TransferRegistry {
    ...
    fn observe_bytes(
        ...
        total: u64,
        at: Instant,
    ) -> Option<crate::rate::Pace> {
        let window = self
            .rates
            .entry(id.clone())
            .or_insert_with(|| crate::rate::Window::new(BYTE_SAMPLE_INTERVAL));
        window.push(at, done);
