...
//! - lines that **scroll off the top** (`Changes::scrollback`) forwarded into
//!   the terminal's native scrollback via `ratatui`'s `insert_before`.
//!
...
//! the whole of this module: pure, no I/O, unit-tested against a real `avt`
//! parser so the colour/attribute/wide-char handling can't silently drift.
...
/// counterparts, so no information is lost.
...
///
/// Two things matter for fidelity and cost:
...
/// - **Trailing blanks are trimmed**: avt pads every line to the full width
///   with default cells; forwarding those into scrollback would tack ~180
///   spaces onto a short `Compiling foo` line. We cut at the last non-default
///   cell. A cell carrying a non-default background (e.g. a highlighted row) is
///   not "default", so genuine trailing colour is preserved.
/// - **Wide-char tails (width 0) are skipped**: the head cell already carries
///   the full glyph; the tail is a placeholder.
...
///
/// This deliberately routes the panel through the *same* cell→span mapping the
/// live child grid uses, so the two halves of the viewport can never drift in
/// colour/attribute handling. The panel is generated with bare `\n` line
/// breaks (`writeln!`), but `avt` is a raw VT — a lone line-feed moves down a
/// row without returning to column 0 — so we normalise `\n` to `\r\n` first,
/// exactly as a terminal's `ONLCR` discipline would for a child's output.
