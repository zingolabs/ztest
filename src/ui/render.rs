...
use super::layout::*;
use super::text::{byte_pair, byte_rate, column_width, compact, format_elapsed, meter, thousands};
...
mod tests {
    ...
    fn at_tiers(sets: &[(QosClass, u32)]) -> Vec<crate::qos::schedule::PlannedTest> {
        sets.iter()
            .flat_map(|&(class, n)| {
                std::iter::repeat_n(
                    crate::qos::schedule::PlannedTest {
                        class,
...
";
    ...
    fn live_panel_with_unknown_capacity_says_so_instead_of_a_zero_gauge() {
        ...
        let plan = schedule::plan(&at_tiers(&[(QosClass::Basic, 2)]), None);
        let snapshot =
    ...
    /// [`plot_channels`](crate::sync::plot_channels) = the watcher's real channel set
