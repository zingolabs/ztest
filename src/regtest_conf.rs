...
network = 'Regtest'{ephemeral_line}{metrics_line}
...
mod tests {
    ...
    #[test]
    fn ephemeral_tuning_renders_one_key_and_omits_it_when_off() {
        let zai = |ephemeral| {
            regtest_zainod_conf(
                "0.1.0".parse().unwrap(),
                "fetch",
        ...
        // Off = byte-identical to every pre-existing topology's render
        assert!(!zai(false).contains("ephemeral_finalised_state"));
        assert!(zai(true).contains("\nephemeral_finalised_state = true\n"));
        // Top-level key → must land before the first table header
        let on = zai(true);
        assert!(
            on.find("ephemeral_finalised_state").unwrap() < on.find("[grpc_settings]").unwrap()
        );
    }
