...
- Height = one frontier family, in probe and panel alike (zaino's `zaino_fetch_height`: per block;
  per-index `zaino_index_finalized_height` steps once per commit batch and carries no blocks/s)
